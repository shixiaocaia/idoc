...
- Unicode（Unicode.org)，包括了世界上所有文书体系的全部字符等，每个字符被赋予一个叫做Unicode码点的标准数字，在Go中称为**文字符号**（rune)，数据类型是int32，因此rune类型作为int32类型的别名
  ...
  - rune 是一种类型别名（type alias），表示 int32 类型的 Unicode 码点（code point）。rune 类型通常用于表示 Unicode 字符。
- 相关包
...
- 裸返回，会按照返回列表里的顺序，将每个结果返回，应当保守使用这一特性

- 文件结束的标志是EOF，在io包里，`io.EOF`
...
- 匿名函数
  ...
  - 在func后面没有函数名，是一个表达式
  - 当匿名函数需要进行递归时，必须先声明一个变量然后将匿名函数赋予给这个变量，不能合并操作。
...
ans := make([][]int, n)
//创建一个长度为n的空的二维整型切片，即初始状态下没有任何元素。
for i := range ans {
    ans[i] = make([]int, n)
}
//对于每一个元素，也创建一个长度为n的空的一维整型切片，即在二维数组中创建n个空的一维数组。

```

- 数组长度是数组类型的一部分，所以大小不同的数组不能直接赋值
- 数组声明时候一定要给定大小
...
- 属性cap表示最大扩张容量，`cap = max - low`，当然这个扩张容量也不是无限的扩张,它是受到了底层数组array的长度限制,超出了底层array的长度就会panic，一般是slice的起始元素到底层数组的最后一个元素间的个数。
- 两个slice不能直接比较是否拥有相同的元素
- 比较一个slice类型是否为空，应该使用`len(s) == 0`，值为nil的slice长度和容量都是0，但是也有非nil的slice长度和容量是0，导致非空无法检测出来。
...
- 实际中一次并不清楚是否扩大了内存大小，通常我们默认将append的调用结果再次赋值给传入append函数的slice。
...
//删除
delete(m, 1) //删除map中1这个元素值
```

- map是一个链式hash表，初始化随机一个开始位置
- 键值对元素的无序集合，在这个集合中，Key是唯一的
- map元素不是一个变量，不可以获取他的地址（map的增长可能导致已有元素被重新散列到新的存储位置，之前的地址就无效了，所以不可以获取
- 默认map中元素的迭代顺序是不固定的，不同的实现方法会使用不同的散列算法
  - sort包提供给键排序的算法，例如使用Strings函数
- map不可比较，唯一合法就是和nil比较
...
- 不同的成员变量顺序，代表不同的结构体变量

- 结构体中成员变量名大写，代表是可以导出的，结构体中可以包含可导出和不可导出的变量

- 结构体成员变量只能包含自身类型的指针，不可以直接包含自己，在树和链表中有体现
...
- 大型的结构体通常都使用结构体指针的方式直接传递给函数或者从函数中返回，此外修改结构体中的内容，通常也是通过指针进行。
...
- 如果结构体中成员变量都是可比较的，那么整体就是可以比较的，整个结构体变量比较是按照顺序比较的。可以比较的结构体类型可以作为map的键类型。

- 匿名成员：不带名称的结构体成员，这种只能是一个命名类型或者指向命名类型的指针。
...
JavaScript对象表示法（JSON）是一种发送和接受格式化信息的标准，XML也是。但是JSON简单、可读性强并且支持广泛，所以用的多。

- 只有可导出的成员可以转换为JSON字段，因此GO结构体里面的所有成员都定义为首字母大写
